...
export function analyseChangeRisk(description: string): ChangeRiskAnalysis {
  ...
  const matchedSignals = SIGNAL_PATTERNS.filter(({ patterns }) =>
    patterns.some((pattern) => pattern.test(normalisedDescription)),
  ).map(({ signal }) => signal);
  ...
  return {
    riskLevel,
    impactedAreas:
      impactedAreas.length > 0 ? impactedAreas : ["Application behaviour"],
    recommendedTesting,
    rationale,
  };
}
...
function getRiskLevel(score: number, description: string): RiskLevel {
  const hasBroadChangeLanguage =
    /\b(refactor|rewrite|replace|overhaul|migrate|architecture|critical|security)\b/i.test(
      description,
    );
...
function buildRationale(
  ...
  if (score >= 7) {
    rationale.push(
      "Multiple risk signals overlap, increasing the chance of regressions across boundaries.",
    );
  }

  if (/\b(new|add|create|enable)\b/i.test(description)) {
    rationale.push(
      "New behaviour should be covered for successful use, failure handling, and unintended access paths.",
    );
  }
